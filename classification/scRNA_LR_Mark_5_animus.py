# -*- coding: utf-8 -*-
"""
Created on Tue Nov 29 20:44:28 2022
...
scores_lr= cross_validate(clf_lr, Xtrain, 
                        ytrain, cv=5,
...
ypred_test_lr = clf_lr.predict(Xtest)

acc_test_lr = accuracy_score(ytest, ypred_test_lr)

prec_test_lr = precision_score(ytest, ypred_test_lr)  # Precision decrease a lot! (form 86% to 56%)

rec_test_lr = recall_score(ytest, ypred_test_lr)

mcc_test_lr = matthews_corrcoef(ytest, ypred_test_lr)

tn_test_lr, fp_test_lr, fn_test_lr, tp_test_lr = confusion_matrix(ytest, ypred_test_lr).ravel()

print('Accuracy on validation:'+str(acc_test_lr))
print('Precision on validation:'+str(prec_test_lr))
print('Recall on validation:'+str(rec_test_lr))
print('MCC on validation:'+str(mcc_test_lr))
spec_lr = tn_test_lr / (tn_test_lr + fp_test_lr)
...
ConfusionMatrixDisplay.from_predictions(ytest, ypred_test_lr)
plt.show()
...
ind_myeloid_benign_d0 = [i for i, x in enumerate(anno328d0.PredictionRefined) if 'normal' in x]

ind_myeloid_malign_d37 = [i for i, x in enumerate(anno329d37.PredictionRF2) if 'malignant' in x]
ind_myeloid_benign_d37 = [i for i, x in enumerate(anno329d37.PredictionRF2) if 'normal' in x]
...
iGenes_d0 = [i for i, x in enumerate(df_d0['Gene']) if x in anndata_inner.var_names]
iGenes_d37 = [i for i, x in enumerate(df_d37['Gene']) if x in anndata_inner.var_names]
...
andata_val.var_names = anndata_inner.var_names
...
barcodes2class_1 = dict(zip(pd.concat((anno328d0.Cell, anno329d37.Cell)),
                          pd.concat((anno328d0.PredictionRefined, 
                                     anno329d37.PredictionRefined))))
...
ConfusionMatrixDisplay.from_predictions(y_val, ypred_val_lr)
plt.show()
...
ypred_lr_Fan1 = clf_lr.predict(x_Fan1)
labels_Fan1=[]

for p in ypred_lr_Fan1:
    labels_Fan1.append(np.argmax(p))
